...
function ProfilePage() {
    ...
    const onDeletePicture = async () => {
        if (window.confirm('Are you sure you want to delete your profile picture?')) {
           try {
                ...
                console.error(err);
                alert(err.response?.data.msg || 'Failed to delete profile picture');
            } 
        }
  ...
  return (
    <div>
        <div className='bg-gray-800 text-white p-8 rouded-lg shadow-md mb-8 text-center'>
            ...
            {loggedInUser && loggedInUser.username === profile.username && (
                <div>
                    <form onSubmit={onUpload} className='mt-4'>
                        <input type="file" onChange={onFileChange} className='text-sm text-black cursor-pointer border bg-gray-300 rounded-sm p-2' />
                        <button type="submit" className='bg-green-600 px-4 py-2 rounded-md text-white font-bold text-sm ml-2 hover:bg-green-700 '>
                            Upload
                        </button>
                    </form>
                    {profile.profilePicture && (
                        <button onClick={onDeletePicture} className='bg-red-600 px-4 py-2 mt-4 rounded-md text-white font-bold text-sm'>
                            Delete Picture
                        </button>
                    )}
                </div>
