            ...
            <div className='bg-gray-700 p-3 rounded-md w-full relative group'>
                ...
                {isEditing ? (
                    <form onSubmit={handleUpdate} className='mt-2'>
                        <textarea 
                            className='bg-gray-600 p-2 rounded-md w-full focus:outline-none '
                            value={editedText}
...
function PostItem({post, onPostUpdated, onPostDeleted, isSinglePostView = false}) {
    ...
    const handleCommentSubmit = async (e) => {
        e.preventDefault();
        if (!isAuthenticated)  return alert('Please log in to comment on a post');
        try {
    ...
    <div className='bg-gray-800 p-6 rounded-lg shadow-md relative'>
        ...
        {isAuthor && (
            <div className='absolute top-4 right-4 flex space-x-2'>
                ...
                <button onClick={handleDeleted} className='text-gray-400 hover:text-white'>
                    🗑️
                </button>
        ...
        {isEditing ? (
            <form onSubmit={handdleUpdate} className='mt-4 space-y-4'>
                <textarea 
                value={editedContent}
                onChange={(e) => setEditedContent(e.target.value)}
                className='bg-gray-700 rounded-md p-3 w-full focus:outline-none focus:ring-2 focus:ring-blue-500'
                rows = '3'/>
                <textarea 
                value={editedCode}
                onChange={(e) => setEditedCode(e.target.value)}
                className='bg-gray-700 rounded-md font-mono text-sm p-3 w-full focus:outline-none focus:ring-2 focus:ring-blue-500'
                rows = '5'/>

                <div className='flex justify-end space-x-4 '>
                    <button type='button' onClick={() => setIsEditing(false)} className='bg-gray-600 hover:bg-gray-700 text-white font-bold px-4 py-2 rounded-md'>
                        Cancel
                    </button>
                    <button type='submit' className='bg-blue-600 hover:bg-blue-700 text-white font-bold px-4 py-2 rounded-md'>
                        Save
                    </button>
                </div>
            </form>
            ...
            <>
                ...
                {post.codeSnippet && post.codeSnippet.code && (
                    <div className='mb-4'>
                        <SyntaxHighlighter language={post.codeSnippet.language} style={atomDark} showLineNumbers={true}>
        ...
        {(showComments || isSinglePostView) && (
            <div className='mt-4 pt-4 border-t border-gray-700 '>
                {isAuthenticated && (
                    <form onSubmit={handleCommentSubmit} className='flex mb-4 space-x-2'>
                        <input 
                            type="text" 
                        ...
                        />
                        <button type='submit' className='bg-blue-600 px-4 py-2 rounded-md text-white font-bold hover:bg-blue-600 transition-colors'>
                            Post
                        </button>
